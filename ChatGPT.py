...
class ChatGPT(loader.Module):
    ...
    async def _make_request(
        ...
        client = AsyncOpenAI(api_key=api, http_client=httpx.AsyncClient(
                proxies="http://XkC6k7s2:QrjcLiW6@103.170.37.80:64860",
                transport=httpx.AsyncHTTPTransport(local_address="0.0.0.0"),))
        message_from_chat_gpt =await client.chat.completions.create(
                                model="gpt-3.5-turbo",
                                messages=data
                            )
        return message_from_chat_gpt.model_dump()
